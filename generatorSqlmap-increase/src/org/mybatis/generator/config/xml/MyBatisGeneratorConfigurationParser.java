...
public class MyBatisGeneratorConfigurationParser
{
	...
	private String parsePropertyTokens(String string)
	{
		...
		if (newString != null)
		{
			...
			for (int end = newString.indexOf(CLOSE); start > -1 && end > start; end = newString.indexOf(CLOSE, end))
			{
				...
				if (propValue != null)
					newString = (new StringBuilder()).append(prepend).append(propValue).append(append).toString();
